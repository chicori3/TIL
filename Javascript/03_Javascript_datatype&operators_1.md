...
- 2.  개발자 도구에서 객체를 출력해보면 **proto프로퍼티**가 있는데 이 프로퍼티가 **프로토타입 객체**를 가리킨다.
...
참고로 proto 프로퍼티가 가리키는 객체가 바로 Object.Prototype 이며, toString(), valueOf() 등과 같은 모든 객체에서 호출 가능한 자바스크립트 기본 내장 메서드가 포함되어 있다. 그 결과 foo 객체는 foo.toString()과 같이 자신의 프로토타입인 Object.prototype 객체에 포함된 다양한 메서드를 마치 자신의 프로퍼티인 것처럼 상속받아 사용할 수 있다.
