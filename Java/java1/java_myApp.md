...
- 동일한 코드를 변수로 지정해 데이터가 변할 때마다 새로 작성하는 수고를 줄임
- 깔끔하고 간결한 코드는 덤
...
- 자바의 메서드는 클래스 내부에만 존재하는 함수이다
- 똑같은 내용을 반복할 때 메서드를 사용한다
- 어떠한 입력값을 넣었을 때 어떤 리턴값을 돌려준다라는 식으로 작성한다
...
- 인스턴스는 클래스를 복제하여 같은 메서드에 다른 상태를 갖도록한 것이다
- new 키워드로 복제할 수 있다
